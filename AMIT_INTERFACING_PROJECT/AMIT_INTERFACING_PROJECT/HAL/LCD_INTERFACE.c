...
extern void HIGH_TO_LOW_PULSE(void)
{
...
extern void LCD_FORCE_CURSOR_POSITION(uint8_t u8y_position , uint8_t u8Line_number)
{
	uint8_t u8Command_number = 0 ;
	
	if (u8Line_number == LINE1)
	{
		u8Command_number = (FIRST_LINE_START) + u8y_position ;
		LCD_COMMAND(u8Command_number);
	} 
	else if(u8Line_number == LINE2)
	{
		u8Command_number = (SECOND_LINE_START) + u8y_position ;
		LCD_COMMAND(u8Command_number);
	}
	else
	{
